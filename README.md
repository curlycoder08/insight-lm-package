...
5.  **Add N8N Webhooks to Supabase Secrets**
    ...
    * **IMPORTANT:** Before adding these secrets, ensure ALL n8n workflows are ACTIVE. In each n8n workflow, toggle the "Active" switch in the top-right corner. Only active workflows have valid production URLs.
...
If you encounter "Document processing failed" errors in the frontend, this typically indicates n8n webhook configuration issues:

1. **Check Workflow Status:**
   - Navigate to your n8n instance
   - Open the "InsightsLM - Upsert to Vector Store" workflow
   - Ensure the workflow is **ACTIVE** (toggle switch in top-right corner)
   - Copy the **Production URL** from the webhook node

2. **Update Supabase Secret:**
   - In Supabase dashboard, go to `Edge Functions` -> `Secrets`
   - Update `DOCUMENT_PROCESSING_WEBHOOK_URL` with the copied Production URL

### Notebook Content Generation Failed Error

If notebook content generation fails:

1. **Check Workflow Status:**
   - Navigate to your n8n instance
   - Open the "InsightsLM - Generate Notebook Details" workflow
   - Ensure the workflow is **ACTIVE**
   - Copy the **Production URL** from the webhook node

2. **Update Supabase Secrets:**
   - In Supabase dashboard, go to `Edge Functions` -> `Secrets`
   - Update `NOTEBOOK_GENERATION_URL` with the copied Production URL
   - Verify `NOTEBOOK_GENERATION_AUTH` matches the Header Auth credential in n8n

### General Webhook Troubleshooting Steps
...
2. **Check Webhook URLs:**
   - Each active workflow should have a webhook node with a Production URL
   - Production URLs should be in format: `https://your-n8n-instance.com/webhook/[webhook-id]`
   - Test URLs (for development) won't work in production

3. **Verify Authentication:**
   - All webhooks use Header Authentication
   - The `NOTEBOOK_GENERATION_AUTH` secret in Supabase must match the credential ID in n8n
   - Default credential name is "MyNoteBookLM Auth"

### Vector Embedding Dimension Error

If you encounter "expected 1536 dimensions, not 768" error:

1. **Database Schema Fix:**
   - Run the `fix_embedding_dimensions.sql` migration in Supabase
   - This updates the vector column to use 768 dimensions instead of 1536

2. **Verify n8n Embedding Model:**
   - In your n8n "Upsert to Vector Store" workflow
   - Check the "Embeddings OpenAI" node configuration
   - Ensure it's using a model that produces 768-dimensional vectors
   - Common models and their dimensions:
     - `text-embedding-ada-002`: 1536 dimensions
     - `text-embedding-3-small`: 1536 dimensions  
     - `text-embedding-3-large`: 3072 dimensions
     - Other models may use 768 dimensions

3. **Alternative Solution:**
   - If you want to use 1536-dimensional embeddings, update your n8n workflow to use `text-embedding-ada-002` or `text-embedding-3-small`
   - The database schema supports 1536 dimensions by default
